...
eBayResource.prototype = {
    ...
    _oauthToken: function (data, options, callback) {
        ...
        var REDIRECT_URI = data.redirectURI;
        if (isSandbox === true) {
        ...
        if (options.action === 'GET_REDIRECT_URL') {
            var authorizationUri = oauth2.authorizationCode.authorizeURL({
                redirect_uri: EBAY_APP.REDIRECT_URI,
                scope: EBAY_APP.REDIRECT_URI,
            });
            return callback(null, {redirectUrl: authorizationUri});
        } else if (options.action === 'GET_USER_TOKEN') {
            var tokenOptions = {
                code: data.code,
                redirect_uri: EBAY_APP.REDIRECT_URI,
                scope: EBAY_APP.REDIRECT_URI
            };
            var tokenStore = oauth2.authorizationCode.getToken(tokenOptions);
            return callback(null, tokenStore);
        }
    },
