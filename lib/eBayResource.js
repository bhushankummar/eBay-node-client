...
eBayResource.prototype = {
    ...
    _request: function (method, path, data, auth, options, callback) {
        ...
        // adding trading api, AG
        if (options.contentType === 'application/x-www-form-urlencoded; charset=UTF-8') {
            headers['Content-Type'] = options.contentType;
            headers['Accept'] = '*/*';
            headers['X-EBAY-API-IAF-TOKEN'] = self._eBay.getApiField('userToken');
            options.headers['X-EBAY-API-IAF-TOKEN'] = self._eBay.getApiField('userToken');
            self.body = data.xml;
            headers['call'] = data.method;
            headers["X-EBAY-API-CALL-NAME"] = data.method;
            options.headers['call'] = data.method;
            options.headers["X-EBAY-API-CALL-NAME"] = data.method;
            headers['Content-Length'] = self.body.length;
            options.headers['Content-Length'] = self.body.length;
