...
// var debug = require('debug')('eBay:eBayMethod');
...
function eBayMethod(spec) {
    ...
    return function () {
        ...
        return this.wrapTimeout(new Promise((function (resolve, reject) {
            for (var i = 0, l = urlParams.length; i < l; ++i) {
                ...
                if (!arg) {
                    ...
                    return reject(new Error(
                      'eBay: Argument "' + urlParams[i] + '" required, but got: ' + arg +
            ...
            if (args.length) {
                var err = new Error('eBay: Unknown arguments (' + args + ').');
                return reject(err);
            }
            ...
            options.apiType = spec.apiType || '';
            options.api = spec.api || '';
            options.call = spec.call || '';
            options.variation = spec.variation || '';
            self._request(requestMethod, requestPath, data, opts.auth, options, requestCallback);
        }).bind(this)), callback);
    };
}

module.exports = eBayMethod;
